...
\hline
Focal \newline Length & $\beta$ & 2 & $\frac{1}{2}\abs{\frac{\eta}{\lambda_2}}$
\\ 
\hline
Focus \newline & $\vec{F}$ & \myvec{2\\0} & $\vec{F}$ = $\vec{c}$ +  \newline ${\frac{-2\eta\myvec{1 & 0}}{4}}^{T}$ \newline \\
\hline
Axis &  & $\myvec{0 & 1}\vec{x}=0$ & $k(\vec{V}\vec{c}+\vec{u})^{T}\vec{x} = 0$ \\
\hline
Direct- \newline rix &  & &$(\vec{V}\vec{c}+\vec{u})^T(\vec{x} +\beta) + \vec{u}^T\vec{c} + \vec{f} = 0$ \newline \\
\hline
Latus \newline rectum & & &$(\vec{V}\vec{c}+\vec{u})^T(\vec{x} -\beta) + \vec{u}^T\vec{c} + \vec{f} = 0$ \\
\hline
End \newline points \newline of latus \newline rectum & $\kappa$ & $\myvec{2 \\ \pm 4}$ & $\vec{u}^T\kappa =$ \newline $-\frac{(\kappa^T\vec{V}\kappa + f )}{2}$ \\
\hline
...
\begin{align}
k(\vec{V}\vec{c}+\vec{u})^{T}\vec{x} = 0 \label{eq:axis}
\end{align}
where,
\begin{align}
    k \in \mathbb{R}
