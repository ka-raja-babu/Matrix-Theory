...
\\
\vec{x} &= \vec{P}+\lambda\vec{m} \quad \brak{\lambda \in \mathbb{R}} \label{eq:14}
\end{align}
Using \eqref{eq:16} and \eqref{eq:14},directrix can be expressed as 
\begin{align}
    \vec{v_1}\vec{x} = -\beta \quad (\text{$\vec{v_1}$ is normal vector})
\end{align}
Similarily,using \eqref{eq:16} and \eqref{eq:14},latus rectum can be expressed as \begin{align}
    \vec{v_2}\vec{x} = \beta \quad (\text{$\vec{v_2}$ is normal vector})
\end{align}
