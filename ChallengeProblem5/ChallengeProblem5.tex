...
Let the eigen vector of $\vec{V}$ having smaller eigen value be $\vec{p}$ .

Now,normal vector of $\vec{p}$ having same magnitude,is given by
\begin{align}
    \vec{p_n}^T\vec{p} &= 0 \\
    \implies \vec{p_n}^T &= \vec{p}^T\myvec{-\vec{e_2} & \vec{e_1}} \quad \brak{\because  \vec{p}^T\myvec{-\vec{e_2} & \vec{e_1}}\vec{p} = 0}
\end{align}
